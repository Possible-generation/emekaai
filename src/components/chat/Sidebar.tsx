    ...
    <aside
      className={`fixed h-full top-0 w-[60%] sm:w-[40%] md:static md:w-[330px] md:top-auto md:left-auto px-5 py-10 transition-all ease-in-out delay-100 z-20 ${
        history.length > 5
          ? "overflow-y-scroll activity mb-10"
          : "overflow-auto"
      } ${openAside ? "left-0" : "-left-full"} bg-[#15411F] text-[#2A2A2A]`}
    >
      <div className="w-full h-full flex flex-col gap-y-5 relative">
        {/* logo */}
        {/* <Link
        ...
        <button
          disabled={conversations.loader}
          onClick={initiateNewChat}
          className="flex flex-row items-center pl-3 gap-2 bg-[#EBEBEB] font-bold w-full text-sm min-h-[42px] rounded-lg cursor-pointer text-[#15411F] disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
        ...
        <div className="relative">
          <input
            className="h-[40px] pl-10 pr-3 rounded-full w-full text-sm md:text-base focus:outline-none bg-[#E4E4E4] text-[#2A2A2A] placeholder:text-[#2A2A2A]"
            placeholder="Search"
            name="searchbar"
            onChange={(e) => setSearchText(e.target.value)}
          />
        ...
        {/* history sign */}
        <div className="uppercase flex flex-row gap-x-1.5 items-center mt-10">
          <BsStars size={20} className="text-[#EBEBEB]" />
          <p className="text-[#EBEBEB]">Chat</p>
        </div>

        {/* line */}
        <hr className="border w-full border-[#B9B9B9]" />
            ...
            queries?.map((chat, index) => {
              return (
                <div
                  ...
                  key={index}
                  className={`flex flex-row gap-x-4 p-1.5 hover:cursor-pointer hover:bg-white hover:text-[#15411F] rounded-lg transition delay-100 ease-in-out lg:gap-x-2 ${
                    currConvId === chat.id
                      ? "bg-white text-[#15411F]"
                      : "text-[#EBEBEB] bg-transparent"
                  }`}
                >
                  <FaRegCommentDots size={22} />
        ...
        {/* logout */}
        <div className="w-full justify-start flex static py-3 h-full items-end">
          <div
            onClick={initiateLogout}
            className="flex w-full items-center capitalize gap-x-2 p-2 rounded-lg hover:cursor-pointer transition text-[#EBEBEB] ease-in-out delay-100 hover:bg-[#5c5c5c2d]"
          >
            <AiOutlineLogout />
            <p>logout</p>
          </div>
